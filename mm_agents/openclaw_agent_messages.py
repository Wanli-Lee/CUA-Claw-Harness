"""OpenClaw agent for WildClawBench — Anthropic Messages API variant.
...
* ``openclaw_agent.OpenClawAgent`` configures pi-ai's ``litellm`` provider
  ...
  runs that go through the MAI LiteLLM gateway directly.
* ``openclaw_agent_chat.OpenClawChatAgent`` configures it with
  ``"api": "openai-completions"`` (Chat Completions API). Used with the
  local cop-api / GitHub Copilot proxy on 127.0.0.1:4141 for non-Claude
...
* ``openclaw_agent_messages.OpenClawMessagesAgent`` (THIS file) configures
  ...
  (Anthropic's native ``/v1/messages`` endpoint). Used with cop-api for
  Claude models — Claude returns ``thinking`` and ``tool_use`` content
...
# MIRROR of mm_agents/openclaw_agent.py:_TOOLING_POLICY (Responses transport).
...
def _resolve_thinking_level(model: str, default: str = "low") -> str:
    """WCB 2026-05-15: env-controllable thinking level for the messages path.
...
def _configure_sh(model: str, base_url: str, api_key: str, gui: bool) -> str:
    ...
    reasoning_flag = "true"
    # thinkingLevel: per-model API constraints + WCB_AGENT_THINKING env override.
...
class OpenClawMessagesAgent:
    ...
    native messages API (``/v1/messages``) — e.g. the local cop-api proxy
    bridging GitHub Copilot Enterprise for Claude models. For the OpenAI
    ...
    def _ensure_plugin_installed(self, env) -> None:
        ...
        # WCB: anthropic.patched.js — only relevant for the messages-API path
...
'''
    ...
    def run(self, env, instruction: str, output_dir: Path,
        ...
        # WCB 2026-05-09: append shared TOOLING_POLICY (anti OCR/browser/CV
        ...
        # run_deep_search_in_osworld.py) still get the policy. GUI mode
        ...
        #
        # WCB 2026-05-15: route through _resolve_thinking_level so the
        # Anthropic messages path also honours WCB_AGENT_THINKING (with
