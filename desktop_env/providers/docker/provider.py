...
class DockerProvider(Provider):
    def __init__(self, region: str):
        ...
        self.vlc_port = None
        self.container = None
        self.environment = {"DISK_SIZE": os.getenv("OSWORLD_DISK_SIZE", "64G"), "RAM_SIZE": os.getenv("OSWORLD_RAM_SIZE", "4G"), "CPU_CORES": os.getenv("OSWORLD_CPU_CORES", "4")}  # Modify if needed
    ...
    def _wait_for_vm_ready(self, timeout: int = 1800):
        ...
        Default 1800s (30min) — cold-boot under heavy concurrent envs IO contention
        on NAS-backed storage can take 15+min for the v3 27GB qcow2.
        """
    ...
    def stop_emulator(self, path_to_vm: str, region=None, *args, **kwargs):
        ...
        if self.container:
            ...
            try:
                ...
                try:
                    log_dir = os.path.join("logs", "docker")
                    os.makedirs(log_dir, exist_ok=True)
                    log_file = os.path.join(log_dir, f"{self.container.short_id}.log")
                    with open(log_file, "w", encoding="utf-8") as f:
                        f.write(self.container.logs().decode("utf-8", errors="replace"))
                    logger.info(f"Docker container logs saved to {log_file}")
                except Exception as e:
                    logger.error(f"Failed to save container logs: {e}")
