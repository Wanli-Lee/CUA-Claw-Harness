...
class DesktopEnv(gym.Env):
    ...
    def _setup_docker_proxy_forwarding(self):
        """Set up socat port forwarding inside Docker container so QEMU VM can reach the proxy.
        
        QEMU VM uses a bridge network (dockerbridge 20.20.20.0/24) where the Docker container
        is at 20.20.20.1. This sets up socat inside the container to forward proxy traffic
        from the container to the Docker bridge gateway (e.g. 172.29.0.1), making the proxy
        accessible from the VM via 20.20.20.1.
        """
        from desktop_env.controllers.setup import get_global_proxy_pool
        proxy_pool = get_global_proxy_pool()
        current_proxy = proxy_pool.get_next_proxy()
        if not current_proxy:
            logger.warning("No proxy available, skipping Docker proxy forwarding")
            return
        ...
        if not container:
            logger.warning("Docker container not available, skipping proxy forwarding")
            return
        
        logger.info(f"Setting up socat forwarding in Docker container: 0.0.0.0:{proxy_port} -> {proxy_host}:{proxy_port}")
        # Install socat inside the container
        install_cmd = "bash -c 'apt-get update -qq > /dev/null 2>&1; apt-get install -y -qq socat > /dev/null 2>&1'"
        try:
            exit_code, output = container.exec_run(install_cmd, detach=False)
            logger.info(f"Docker socat install: exit_code={exit_code}")
        except Exception as e:
            logger.error(f"Failed to install socat in Docker container: {e}")
            return
        
        # Start socat forwarding as a detached background process
        fwd_cmd = f"socat TCP-LISTEN:{proxy_port},fork,reuseaddr,bind=0.0.0.0 TCP:{proxy_host}:{proxy_port}"
        try:
            container.exec_run(fwd_cmd, detach=True)
            import time
            time.sleep(2)
            # Verify it's listening
            exit_code, output = container.exec_run(f"bash -c 'ss -tlnp | grep {proxy_port}'")
            if exit_code == 0:
                logger.info(f"Docker socat forwarding active: {output.decode().strip()}")
            else:
                logger.error(f"Docker socat forwarding not listening on port {proxy_port}")
        except Exception as e:
            logger.error(f"Failed to start socat forwarding: {e}")
            ...
            if task_config is not None:
                if task_config.get("proxy", False) and self.enable_proxy:
                    ...
                    if self.provider_name == "docker":
                        self._setup_docker_proxy_forwarding()
                    self.setup_controller._proxy_setup(self.client_password, is_docker=(self.provider_name == "docker"))
