...
class SetupController:
    ...
    def _proxy_setup(self, client_password: str = "", is_docker: bool = False):
        ...
        Args:
            ...
            is_docker (bool): If True, rewrite proxy host to 10.0.2.2 (QEMU SLIRP gateway)
        """
        retry = 0
        while retry < MAX_RETRIES:
            try:
        ...
        if is_docker:
            # QEMU VM inside Docker uses bridge networking (20.20.20.0/24).
            # Docker container is at 20.20.20.1 on the bridge.
            # socat forwarding is set up in the container by DesktopEnv._setup_docker_proxy_forwarding()
            proxy_host = "20.20.20.1"
            logger.info(f"Docker mode: rewriting proxy host from {current_proxy.host} to {proxy_host}")
        
        # Create a modified proxy for URL formatting
        proxy_url = f"http://{proxy_host}:{proxy_port}"
        if current_proxy.username and current_proxy.password:
            proxy_url = f"http://{current_proxy.username}:{current_proxy.password}@{proxy_host}:{proxy_port}"
        logger.info(f"Setting up proxy: {proxy_host}:{proxy_port}")
        
        # Configure system proxy environment variables  
        upstream_cmd = f"echo 'Upstream http {proxy_host}:{proxy_port}'"
        if current_proxy.username and current_proxy.password:
            upstream_cmd = f"echo 'Upstream http {current_proxy.username}:{current_proxy.password}@{proxy_host}:{proxy_port}'"

        proxy_commands = [
            # Setup askpass for non-interactive sudo
            f"echo '#!/bin/bash' > /tmp/askpass",
            f"echo 'echo {client_password}' >> /tmp/askpass",
            "chmod +x /tmp/askpass",
            
            # Execute commands using SUDO_ASKPASS
            "export SUDO_ASKPASS=/tmp/askpass && sudo -A -k bash -c \"apt-get update -qq\"",  ## TODO: remove this line if ami is already updated
            "export SUDO_ASKPASS=/tmp/askpass && sudo -A -k bash -c \"apt-get install -y tinyproxy\"", ## TODO: remove this line if tinyproxy is already installed
            "export SUDO_ASKPASS=/tmp/askpass && sudo -A -k bash -c \"echo 'Port 18888' > /tmp/tinyproxy.conf\"",
            "export SUDO_ASKPASS=/tmp/askpass && sudo -A -k bash -c \"echo 'Allow 127.0.0.1' >> /tmp/tinyproxy.conf\"",
            f"export SUDO_ASKPASS=/tmp/askpass && sudo -A -k bash -c \"{upstream_cmd} >> /tmp/tinyproxy.conf\"",
        ...
        try:
            ...
            try:
                # We use a simple python command to check connectivity inside the VM
                # Using the controller's execute_python_command method requires the controller to be accessible
                # setup_controller is part of the environment, but here we are inside SetupController.
                # SetupController doesn't have execute_python_command directly, PythonController does.
                # However, SetupController methods are usually called from the env or similar context.
                # We can use _execute_setup with python -c to run the check.
                ...
                # We use _execute_setup to run this command in the VM
                # Note: execute_setup returns output directly or raises error
                check_output = self._execute_setup([" ".join(check_cmd)], shell=True)
                logger.info(f"Internet check passed: {check_output.strip() if check_output else 'OK'}")
            except Exception as e:
                logger.error(f"Internet connectivity check failed: {e}")
                # We don't fail the setup here strictly, but logging it is crucial
                # Or should we raise exception? Given it's a proxy setup, we probably should warn loudly.
                logger.warning("WARNING: Internet access verification failed. The task might fail if it requires internet.")
                
        except Exception as e:
            logger.error(f"Failed to reload environment variables or verify network: {e}")
            proxy_pool.mark_proxy_failed(current_proxy)
            raise
    ...
    def _update_browse_history_setup(self, **config):
        ...
        db_url = "https://hf-mirror.com/datasets/xlangai/ubuntu_osworld_file_cache/resolve/main/chrome/44ee5668-ecd5-4366-a6ce-c1c9b8d4e938/history_empty.sqlite?download=true"
        if not os.path.exists(cache_path):
            max_retries = 3
            downloaded = False
            e = None
